...
class Building(object):
    ...
    def get_state(self) -> tuple:
        ...
        floor_passengers = [[[floor,passenger.get_dir()] for passenger in passengers] for floor, passengers in enumerate(self.floors_information)]
        ...
        for floor_num in range(0, self.max_floor):
            if np.random.random() < prob and len(self.floors_information[floor_num]) < self.max_passengers_in_floor:
                ...
                for p in range(passenger_num):
                    additional_passengers.append(Passenger(now_floor=floor_num, max_floor=self.max_floor, dest=random.choice(list(range(floor_num)) + list(range(floor_num+1,self.max_floor)))))
    ...
    def get_passengers(self, prob : float, passenger_max_num : int = 6):
        ...
        # additional_passengers = []                     # 원본
        additional_passengers = [[] for i in range(self.max_floor)]  # floor 정보 생성 (2차원 리스트)
        for p in range(passenger_num):
            now_floor, dest = map(int, lines[p].split(','))
            additional_passengers[now_floor].append(Passenger(now_floor=now_floor, max_floor=self.max_floor, dest=dest))
            
        for now_floor in range(self.max_floor):
            self.floors_information[now_floor] += additional_passengers[now_floor]
        self.remain_passengers_num += passenger_num        
        f.close()
